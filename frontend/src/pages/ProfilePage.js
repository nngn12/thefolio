...
const ProfilePage = () => {
    ...
    const handleProfile = async (e) => {
        ...
        try {
            ...
            fd.append("name", name);
            fd.append("bio", bio);
            // Ensure the key matches what your backend expects (profilePic)
            ...
            // ✅ CRITICAL: This updates the global AuthContext so the header changes!
            if (updateUser) {
                updateUser(res.data.user || res.data);
            }

            // ✅ Clear local previews so it fetches the fresh URL from the server
            setPic(null);
            setPicPreview(null);
